...
<ReviewLessonButton link="/node-express/lesson12-m1" />
...
own data. The second problem is that the data is stored in the memory of the server
...
is because while we're no longer storing `secretWord` as a global variable,
it is still being stored in the memory of the server.
...
These lines cause the session to be stored in Mongo. The bit about `sessionParms.cookie.secure = true`
...
parts. Then, there is substitution of values, in the `<%= %>` parts, where the equals sign
...
But, the problem is that the `info` and `errors` arrays need to get passed into the EJS file, when the
